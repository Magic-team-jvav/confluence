...
public final class BestiaryEntrySubProvider {
    public static void gather(ModDataMapProvider.Appender<DataMapProvider.Builder<PresetBestiaryEntry, EntityType<?>>> appender) {
//        appender.create()
//                .add(TEBossEntities.EYE_OF_CTHULHU.getKey(), new PresetBestiaryEntry(BestiaryEntry.builder(TEBossEntities.EYE_OF_CTHULHU.get(), TEBossEntities.EYE_OF_CTHULHU.get().getDescriptionId()).maxHealth().knockbackResistance().attackDamage().armor().drops().build()), false);
    }

//    public static class Builder extends DataMapProvider.Builder<PresetBestiaryEntry, EntityType<?>> {
//        public Builder() {
