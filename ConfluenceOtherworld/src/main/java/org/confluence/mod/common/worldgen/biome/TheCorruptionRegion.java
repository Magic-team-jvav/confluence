...
/// 那件事交给区域分配器（噪声场）做就够了 —— 两个区域各有一条独立噪声。
///
...
/// ## 占比
///
/// 盒子开到全陆地之后，**占多少完全由区域分配器的权重决定**：主世界是
/// `原版 25 : 腐化 1 : 猩红 1 : 蘑菇 1`，两个邪恶区域又都会重映射成同一个群系，
/// 所以每个邪恶群系实际约占 `1/28 ≈ 3.6%` 的列（实测约 3.3%），乘陆地占比后约
/// **4.7% 的地表**。嫌多/嫌少就调 `BiomeRegionType#vanillaWeight()`（只有相对比例有意义）。
///
/// 曾经用的是 `温度[0.2, 1.0] + 大陆度[-0.11, 0.55] + 侵蚀度[-1.0, 0.05]`，
/// 四条轴相乘后只剩约 5% 的地表满足，再乘区域占比，实际占比不到 1%，
