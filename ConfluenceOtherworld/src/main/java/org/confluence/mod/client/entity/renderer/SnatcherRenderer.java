...
public final class SnatcherRenderer extends PlantHeadRenderer {
    ...
    private void renderVine(Snatcher entity, float partialTick, PoseStack poseStack, MultiBufferSource buffers, int packedLight) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            renderCrossedSegment(poseStack, vertices, (float) segmentLength, segmentLight, overlay);
            poseStack.popPose();
        }
    }

    private static void renderCrossedSegment(PoseStack poseStack, VertexConsumer vertices, float length, int packedLight, int overlay) {
        renderPlane(poseStack, vertices, length, packedLight, overlay);
        poseStack.mulPose(Axis.YP.rotationDegrees(90.0F));
        renderPlane(poseStack, vertices, length, packedLight, overlay);
    }

    private static void renderPlane(PoseStack poseStack, VertexConsumer vertices, float length, int packedLight, int overlay) {
        ...
        quad(vertices, matrix, normal, -0.0625F, lower, 0.0625F, upper,
                16.0F / 64, 0.0F, 18.0F / 64, 16.0F / 64, packedLight, overlay);
        quad(vertices, matrix, normal, -0.4375F, lower + length * 0.5F, -0.0625F, upper,
                5.0F / 64, 18.0F / 64, 11.0F / 64, 23.0F / 64, packedLight, overlay);
        quad(vertices, matrix, normal, 0.0625F, lower, 0.4375F, lower + length * 0.5F,
                27.0F / 64, 18.0F / 64, 33.0F / 64, 23.0F / 64, packedLight, overlay);
    }
    ...
    private static void quad(VertexConsumer vertices, Matrix4f matrix, Matrix3f normal,
                             ...
                             float u0, float v0, float u1, float v1, int packedLight, int overlay) {
        vertex(vertices, matrix, normal, left, lower, 0.0F, u0, v1, packedLight, overlay);
        vertex(vertices, matrix, normal, right, lower, 0.0F, u1, v1, packedLight, overlay);
        vertex(vertices, matrix, normal, right, upper, 0.0F, u1, v0, packedLight, overlay);
        vertex(vertices, matrix, normal, left, upper, 0.0F, u0, v0, packedLight, overlay);
    }

    private static void vertex(VertexConsumer vertices, Matrix4f matrix, Matrix3f normal, float x, float y, float z, float u, float v, int packedLight, int overlay) {
