    ...
    public static int getLuminance(Entity entity, int returnValue) {
        ...
        if (entity instanceof LivingEntity living) {
            if (living instanceof Player player) {
                luminance = IntegerValue.abxMax(luminance, ClientPacketHandler.getLuminance(player));
            }
            if (living.hasEffect(ModEffects.SHINE)) {
                luminance = IntegerValue.abxMax(luminance, 10);
            }
            // todo 手持发光物品
        }
        return IntegerValue.abxMax(luminance, returnValue);
    }
}
