...
public class SuperSpikyBallProjectileRenderer extends EntityRenderer<SuperSpikyBallProjectile> {
    private static final ResourceLocation TEXTURE = Confluence.asResource("textures/entity/super_spiky_ball_projectile.png");
    private final SpikyBallProjectileModel model;

    public SuperSpikyBallProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new SpikyBallProjectileModel(context.bakeLayer(SpikyBallProjectileModel.LAYER_LOCATION));
    }

    @Override
    public ResourceLocation getTextureLocation(SuperSpikyBallProjectile entity) {
        return TEXTURE;
    }

    @Override
    public void render(SuperSpikyBallProjectile entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
