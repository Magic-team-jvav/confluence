...
public class GoblinTinkererNPC extends BaseNPC implements RangedAttackMob {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(2, new RangedBowAttackGoal<>(this, 1.3, 20, 8));
    ...
    public void performRangedAttack(LivingEntity target, float velocity) {
        ...
        projectile.setCritArrow(getRandom().nextFloat() < 0.3F);
        projectile.shoot(dx, dy + dist * 0.2, dz, 1.5F, 1.0F);
    ...
    public boolean canAttack(LivingEntity target) {
        return getMainHandItem().is(Items.BOW) && super.canAttack(target);
    }
}
