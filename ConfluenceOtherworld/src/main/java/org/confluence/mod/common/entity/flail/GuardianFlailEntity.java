...
public class GuardianFlailEntity extends BaseFlailEntity {
    ...
    private static final EntityDataAccessor<Integer> TARGET_ONE =
            SynchedEntityData.defineId(
                    GuardianFlailEntity.class,
                    EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> TARGET_TWO =
            SynchedEntityData.defineId(
                    GuardianFlailEntity.class,
                    EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> TARGET_THREE =
            SynchedEntityData.defineId(
                    GuardianFlailEntity.class,
                    EntityDataSerializers.INT);
    ...
    public GuardianFlailEntity(
            EntityType<? extends GuardianFlailEntity> type,
            Level level,
            boolean elder
    ) {
        super(type, level);
    ...
    public void tick() {
        ...
        float damage = (float) player.getAttributeValue(
                LibAttributes.getAttackDamage()) / 6.0F;
        for (LivingEntity target : targets) {
            target.hurt(
                    level().damageSources().mobAttack(player),
                    damage);
            level().playSound(
                    null,
                    target.getX(),
    ...
    private List<LivingEntity> findTargets(Player player) {
        ...
        List<LivingEntity> candidates = level().getEntitiesOfClass(
                ...
                target -> target != player
                        && target.isAlive()
                        && LibEntityUtils.canHitEntity(target, this)
                        && canSee(target));
