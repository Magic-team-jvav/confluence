...
/**
 * Boss 蠕虫体节。每 tick 跟随 leader，保持固定间距。
 * 受到伤害时转移给头部 Boss。
 */
public class BossWormPart extends Entity implements WormSegment {
    ...
    public BossWormPart(EntityType<?> type, Level level) {
        super(type, level);
    }

    public BossWormPart(Entity leader, int index, Level level) {
    ...
    public WormSegment getPrev() {
        if (leader instanceof BaseWormBoss head) return head.getSegment(index - 1);
        return null;
    }
    ...
    private void doCollisionAttack() {
        if (!(leader instanceof LivingEntity head) || !head.isAlive()) return;
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (leader instanceof LivingEntity head && head.isAlive()) {
