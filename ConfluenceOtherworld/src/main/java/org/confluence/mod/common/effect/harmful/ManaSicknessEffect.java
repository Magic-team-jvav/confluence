...
public class ManaSicknessEffect extends MobEffect {
    ...
    public static float process(ServerPlayer attacker, DamageSource damageSource, float amount) {
        float multiply = 1.0F;
        MobEffectInstance instance = attacker.getEffect(ModEffects.MANA_SICKNESS);
        if (instance != null && (damageSource.is(Tags.DamageTypes.IS_MAGIC) || attacker.getMainHandItem().is(ModTags.Items.MANA_WEAPON))) {
