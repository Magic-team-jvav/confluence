...
public class TheCrimsonConversionTable extends ConversionTable {
    ...
    protected @Nullable Block getTarget(BlockState source) {
        ...
        if (holder.is(ModTags.Blocks.CRIMSON_CONVERSION_SHORT_GRASS)) return NatureBlocks.CRIMSON_GRASS.get();
        if (holder.is(ModTags.Blocks.CRIMSON_CONVERSION_PACKED_ICE)) return NatureBlocks.RED_PACKED_ICE.get();
        ...
        if (holder.is(ModTags.Blocks.ORES_SAPPHIRE)) return OreBlocks.FLESHIFICATION_SAPPHIRE_ORE.get();
        if (holder.is(ModTags.Blocks.ORES_AMETHYST)) return OreBlocks.FLESHIFICATION_AMETHYST_ORE.get();
        ...
        if (holder.is(ModTags.Blocks.ORES_TUNGSTEN)) return OreBlocks.FLESHIFICATION_TUNGSTEN_ORE.get();
        if (holder.is(Tags.Blocks.ORES_GOLD)) return OreBlocks.FLESHIFICATION_GOLD_ORE.get();
        if (holder.is(ModTags.Blocks.ORES_PLATINUM)) return OreBlocks.FLESHIFICATION_PLATINUM_ORE.get();
        if (holder.is(ModTags.Blocks.ORES_DEMONITE)) return OreBlocks.FLESHIFICATION_DEMONITE_ORE.get();
        if (holder.is(ModTags.Blocks.ORES_CRIMTANE)) return OreBlocks.FLESHIFICATION_CRIMTANE_ORE.get();
