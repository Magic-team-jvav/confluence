...
public final class PrimeEnderDragonRenderer
    ...
    public PrimeEnderDragonRenderer(
    ...
    private static void renderLaser(
        ...
        double renderX = Mth.lerp(
                partialTick, dragon.xo, dragon.getX());
        double renderY = Mth.lerp(
                partialTick, dragon.yo, dragon.getY());
        double renderZ = Mth.lerp(
                partialTick, dragon.zo, dragon.getZ());
        Vec3 origin = dragon.getLaserOrigin(partialTick);
        float pitch = Mth.rotLerp(
                partialTick, dragon.xRotO, dragon.getXRot());
        float yaw = Mth.rotLerp(
                partialTick, dragon.yRotO, dragon.getYRot());
        ...
        renderBeamLayer(
                poseStack.last(),
                buffers.getBuffer(
                        RenderType.beaconBeam(BEAM_TEXTURE, false)),
                range,
                CORE_HALF_WIDTH,
                vOffset,
                112, 54, 255, 255);
        renderBeamLayer(
                poseStack.last(),
                buffers.getBuffer(
                        RenderType.beaconBeam(BEAM_TEXTURE, true)),
                range,
                GLOW_HALF_WIDTH,
                vOffset + 0.25F,
                172, 102, 255, 150);
    ...
    private static void renderBeamLayer(
        ...
        beamQuad(
                pose, consumer, length,
                -halfWidth, -halfWidth,
                halfWidth, -halfWidth,
                0.0F, 1.0F, vOffset, vEnd,
    ...
    private static void beamQuad(
        ...
        vertex(
                pose, consumer,
                firstX, length, firstZ,
                minU, maxV,
                red, green, blue, alpha);
        vertex(
                pose, consumer,
                secondX, length, secondZ,
                maxU, maxV,
                red, green, blue, alpha);
        vertex(
                pose, consumer,
                secondX, 0.0F, secondZ,
                maxU, minV,
                red, green, blue, alpha);
    }
