...
public class BestiaryEntry {
    public static final Codec<BestiaryEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.INT.lenientOptionalFieldOf("drops", 0).forGetter(BestiaryEntry::getDrops)
    ).apply(instance, BestiaryEntry::new));
    public static final StreamCodec<ByteBuf, BestiaryEntry> STREAM_CODEC = StreamCodec.composite(
    ...
    public transient EntityType<?> type;
    private transient Coins coins;

    public BestiaryEntry() {}

    private BestiaryEntry(int killedByCount, float maxHealth, float knockbackResistance, float attackDamage, float armor, int drops) {
