...
final class SwordAttackGoal extends SummonGoal<SummonSword> {
    ...
    @Override
    public void tick() {
        Vec3 direction = summon.targetPosition().subtract(summon.position()).normalize();
        ...
        Vec3 movement = angle < 0.05 ? summon.velocity().add(direction).normalize().scale(0.6) : summon.velocity().scale(0.9);
