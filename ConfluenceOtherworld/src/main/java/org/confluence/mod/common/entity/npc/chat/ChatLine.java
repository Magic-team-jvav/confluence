...
public record ChatLine(NPCChat chat, TradeCondition condition, int cooldownTicks) {
    public static final Codec<ChatLine> CODEC = RecordCodecBuilder.create(b -> b.group(
            ...
            TradeCondition.CODEC.optionalFieldOf("condition").forGetter(l -> l.condition instanceof AlwaysTrueCondition ? Optional.empty() : Optional.of(l.condition)),
            Codec.INT.optionalFieldOf("cooldown_ticks", 6000).forGetter(ChatLine::cooldownTicks)
    ).apply(b, (chat, cond, cd) -> new ChatLine(chat, cond.orElse(AlwaysTrueCondition.INSTANCE), cd)));

    public boolean canTrigger(ServerPlayer player, BaseNPC npc) {
