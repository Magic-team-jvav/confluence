...
/// 生物恶魔镰刀的客户端渲染器。
///
/// 与玩家武器弹幕共享模型和贴图，但读取各自独立的旋转状态，
/// 从而不让客户端表现反向耦合玩家法力弹幕的运行时实现。
public final class HostileDemonScytheProjectileRenderer extends EntityRenderer<HostileDemonScytheProjectile> {
    ...
    public HostileDemonScytheProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new DemonScytheProjectileModel(context.bakeLayer(DemonScytheProjectileModel.LAYER_LOCATION));
    }

    @Override
    public ResourceLocation getTextureLocation(HostileDemonScytheProjectile entity) {
        return TEXTURE;
    }

    @Override
    public void render(HostileDemonScytheProjectile entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
