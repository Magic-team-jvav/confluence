...
public abstract class FlyingSummon extends SummonInstance {
    private static final double MOMENTUM_DAMPING = 0.9;
    ...
    protected final void moveToward(Vec3 destination, double acceleration, double maximumSpeed) {
        applyForceToward(destination, null, acceleration, maximumSpeed);
    }
    ...
    protected final void moveToward(Vec3 destination, Vec3 lookAtPosition, double acceleration, double maximumSpeed) {
        applyForceToward(destination, lookAtPosition, acceleration, maximumSpeed);
    }
    ...
    private void applyForceToward(Vec3 destination, Vec3 lookAtPosition, double acceleration, double maximumSpeed) {
        ...
        Vec3 force = distance < 1.0E-6 ? Vec3.ZERO
                : offset.scale(Math.min(distance * acceleration, acceleration * 10.0) / distance);
        ...
        if (distance < 0.05 && nextVelocity.lengthSqr() < 1.0E-4) nextVelocity = Vec3.ZERO;
    ...
    protected final void moveBy(Vec3 movement) {
        float targetYaw = movement.horizontalDistanceSqr() < 1.0E-8 ? currentPose().yaw()
                : (float) Math.toDegrees(Math.atan2(-movement.x, movement.z));
        float targetPitch = movement.lengthSqr() < 1.0E-8 ? currentPose().pitch()
                : (float) Math.toDegrees(Math.asin(-movement.normalize().y));
        float yaw = currentPose().yaw() + Mth.clamp(Mth.wrapDegrees(targetYaw - currentPose().yaw()), -20.0F, 20.0F);
        float pitch = currentPose().pitch() + Mth.clamp(Mth.wrapDegrees(targetPitch - currentPose().pitch()), -15.0F, 15.0F);
