...
public class StructureUtils {
    ...
    private static void list8(List<Vector3d> list, BlockPos centerPos, int x, int y, int z, float chance, WorldgenRandom random) {
        for (int i = 0; i < 8; i++) {
            ...
            for (int y = 0; y < radius; y++) {
                ...
                for (int z = 0; z < radius; z++) {
                    if (chance8 >= random.nextFloat() && (x2 + y2 + z * z <= radius2)) {
                        list8(list, centerPos, x, y, z, chance, random);
                    }
                }
            }
        }
