...
public class DesertTigerIdleGoal extends AttachmentEntityGoal<DesertTigerMinion> {

    public DesertTigerIdleGoal(DesertTigerMinion minion) {
        super(minion);
    }
    ...
    public void tick() {
        ...
        double lateral = (minion.getOrder() - (minion.getSameSize() - 1) * 0.5) * 1.5;
        Vec3 idlePos = Vec3.atBottomCenterOf(BlockPos.containing(owner.position().subtract(forward.scale(3.0)).add(right.scale(lateral))));
        ...
        } else if (subtract.horizontalDistance() > 1.0) {
            minion.lookAtPos(idlePos);
            minion.moveTo(idlePos, 0.06F);
        } else {
            minion.lookAtPos(owner.getEyePosition());
        }
        if (subtract.length() > 64.0) {
