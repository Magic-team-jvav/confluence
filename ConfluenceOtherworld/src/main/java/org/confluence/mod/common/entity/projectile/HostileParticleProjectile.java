...
public final class HostileParticleProjectile
    ...
    public HostileParticleProjectile(
            EntityType<? extends HostileParticleProjectile> type,
            Level level,
            Variant variant) {
        super(type, level);
    ...
    public void configure(
        ...
        Vec3 aim = new Vec3(
                ...
                target.getY() - target.getBbHeight() * 0.3F
                        - owner.getY(),
    ...
    @Override
    protected void onSuccessfulHit(
            Mob owner, LivingEntity target) {
        switch (variant) {
            case VILE_SPIT -> target.addEffect(
                    new MobEffectInstance(
                            MobEffects.HUNGER, 100), owner);
    ...
    public enum Variant {
        ...
        GASTROPOD(new DustParticleOptions(
                new Vector3f(
                        1.0F, 0.2F, 0.8F), 1.15F)),
        WALL_OF_FLESH_LASER(new DustParticleOptions(
                new Vector3f(
                        0.72F, 0.08F, 0.62F), 1.35F));
