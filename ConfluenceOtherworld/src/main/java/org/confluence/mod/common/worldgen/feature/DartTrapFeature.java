...
public class DartTrapFeature extends Feature<DartTrapFeature.Config> {
    ...
    public boolean place(FeaturePlaceContext<Config> pContext) {
        ...
        if (LibFeatureUtils.isPosSturdy(level, mutablePos, Direction.UP)) {
            ...
            for (Direction direction : LibUtils.HORIZONTAL) {
                ...
                int h;
                for (h = 1; h <= config.maxDartDistance && LibFeatureUtils.ensureCanWrite(level, copy) && LibFeatureUtils.isPosAir(level, copy); ++h) {
                    copy.move(direction);
                }
                if (!LibFeatureUtils.ensureCanWrite(level, copy)) continue;
                if (h >= 4 && !level.isStateAtPosition(copy, blockState -> blockState.isAir() || blockState.getCollisionShape(level, copy).isEmpty())) {
                    ...
                    boolean b = LibFeatureUtils.safeSetBlock(level, copy, dartTrap, ModFeatures.IS_REPLACEABLE);
