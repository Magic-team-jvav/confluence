...
public class TheCorruptionConversionTable extends ConversionTable {
    ...
    protected @Nullable Block getTarget(BlockState source) {
        ...
        if (holder.is(BlockTags.LOGS)) return NatureBlocks.EBONY_LOG_BLOCKS.getLog().get();
        if (holder.is(BlockTags.LEAVES)) return NatureBlocks.EBONY_LOG_BLOCKS.getLeaves().get();
