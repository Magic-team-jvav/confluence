...
public class SculkWispIdleGoal extends AttachmentEntityGoal<SculkWispMinion> {

    private Vec3 offset = Vec3.ZERO;

    public SculkWispIdleGoal(SculkWispMinion minion) {
        super(minion);
    }
    ...
    public void start() {
        ...
        Vec3 center = ownerCenter.add(0, owner.getBbHeight() + 1, 0).add(Vec3.ZERO.offsetRandom(minion.getRandom(), owner.getBbWidth() * 2));
    ...
    public void tick() {
        ...
        if (minionPos.distanceTo(targetPos) > 0.5) {
            minion.addVelocity(targetPos.subtract(minionPos).normalize().scale(0.01 * Math.min(1 + targetPos.distanceTo(minionPos) * 0.05, 8)));
