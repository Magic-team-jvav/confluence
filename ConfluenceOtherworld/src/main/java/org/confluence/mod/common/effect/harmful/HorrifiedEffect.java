...
public class HorrifiedEffect extends PortMobEffect {
    ...
    public static void bind(
            LivingEntity living, WallOfFlesh wall) {
        living.getPersistentData().putUUID(
                WALL_UUID_TAG, wall.getUUID());
    }

    public static boolean isBoundTo(
            LivingEntity living, WallOfFlesh wall) {
        CompoundTag data = living.getPersistentData();
        return data.hasUUID(WALL_UUID_TAG)
                && wall.getUUID().equals(
                data.getUUID(WALL_UUID_TAG));
    }

    public static @Nullable WallOfFlesh resolve(
            LivingEntity living) {
        ...
        return entity instanceof WallOfFlesh wall
                && wall.isAlive() ? wall : null;
    }
    ...
    public void applyEffectTick(
        ...
        if (living.level().dimension()
                != wall.level().dimension()) {
            living.kill();
            return;
        }
        if (!living.getBoundingBox().intersects(
                wall.getPursuitBox().inflate(12.0))) {
            living.addEffect(new MobEffectInstance(
                    ModEffects.THE_TONGUE.get(), 60), wall);
        }
    }
